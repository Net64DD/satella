import type { DiscordGuild, OAuthToken, OAuthUser } from "../types/discord";
...
class DiscordOAuthService {
  ...
  async getAccessToken(code: string): Promise<OAuthToken> {
    const raw = await (
      await fetch(`${process.env.DISCORD_API_URL}/oauth2/token`, {
        method: "POST",
  ...
  async getUser(oauth: OAuthToken): Promise<OAuthUser> {
    const raw = await (
      await fetch(`${process.env.DISCORD_API_URL}/users/@me`, {
        headers: {
  ...
  async getGuilds(oauth: OAuthToken): Promise<DiscordGuild[]> {
    const raw = await (
      await fetch(`${process.env.DISCORD_API_URL}/users/@me/guilds`, {
        headers: {
          authorization: `${oauth.token_type} ${oauth.access_token}`,
        },
      })
