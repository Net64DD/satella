...
export const createSessionAndUser = async (code: string) => {
    ...
    if (!token || !token.access_token) {
        throw new ErrorResponse(Responses.INVALID_CREDENTIALS, 'Invalid access token');
    }
    ...
    const session = new UserSession({
        ...
        deviceLinkCode: Array(6).fill(0).map(() => Math.floor(Math.random() * 10)).join(''),
        isLinked: false,
        lastUsed: new Date(),
        createdAt: new Date(),
...
export const linkUserSession = async (deviceLinkCode: string, deviceId: string) => {
...
export const getUserSession = async (token: string, refreshToken?: string): Promise<Session> => {
    const session = await UserSession.findOne({
        token,
        isLinked: true,
    });
    ...
    if (!session) {
        throw new ErrorResponse(Responses.UNAUTHORIZED, 'Invalid session or not linked');
    }
