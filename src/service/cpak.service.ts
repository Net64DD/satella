...
export const createControllerPak = async (
  userId: string,
): Promise<ControllerPak> => {
  ...
  console.debug(
    `User ${user.ulid} has ${existing.length} Controller Paks, max allowed is ${maxSlots}`,
  );
  if (existing.length >= maxSlots) {
    throw new ErrorResponse(
      Responses.BAD_REQUEST,
      "Maximum number of Controller Paks reached",
    );
  }
...
export const deleteControllerPak = async (
  userId: string,
  pakId: string,
): Promise<void> => {
  const user = await User.findOne({ ulid: userId });
...
export const uploadControllerPak = async (
  userId: string,
  pakId: string,
  buffer: Buffer,
) => {
  ...
  console.debug(
    `Uploading Controller Pak for user ${user.ulid}, pakId: ${pakId}`,
    buffer,
  );
  ...
  if (buffer.length > maxSize) {
    throw new ErrorResponse(
      Responses.BAD_REQUEST,
      "Controller Pak data exceeds maximum size",
    );
  }
...
export const updateControllerPak = async (
  userId: string,
  pakId: string,
  data: Partial<ControllerPak>,
) => {
  const user = await User.findOne({ ulid: userId });
  if (!user) {
    throw new ErrorResponse(Responses.NOT_FOUND, "User not found");
...
export const getControllerPaks = async (
  userId: string,
): Promise<ControllerPak[]> => {
  const user = await User.findOne({ ulid: userId });
  if (!user) {
  ...
  const list = await ControllerPak.find({
    $or: [{ ownerId: user.ulid }, { access: { $in: [user.ulid] } }],
  });
