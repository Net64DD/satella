...
export const searchFriends = async (
  ...
  const friendsList: FriendEntry[] = users.map((friend) => ({
    ...
    pending:
      user.friends.sended.includes(friend.ulid) ||
      user.friends.received.includes(friend.ulid),
  }));
...
export const modifyFriendRequest = async (
  userId: string,
  friendId: string,
  accept: boolean,
) => {
  ...
  if (user.friends.received.includes(friendId)) {
    user.friends.received = user.friends.received.filter(
      (id: string) => id !== friendId,
    );
    friend.friends.sended = friend.friends.sended.filter(
      (id: string) => id !== userId,
    );
  ...
  if (user.friends.sended.includes(friendId) && !accept) {
    user.friends.sended = user.friends.sended.filter(
      (id: string) => id !== friendId,
    );
    await user.save();
    return;
...
export const removeFriend = async (userId: string, friendId: string) => {
  ...
  friend.friends.list = friend.friends.list.filter(
    (id: string) => id !== userId,
  );
  ...
  for (const friendId of [
    ...
    if (friend) {
      friendsList.push({
        ...
        accentColor: friend.accentColor,
        favoriteGames: friend.favoriteGames || [],
        status: user.friends.sended.includes(friendId)
          ? FriendRequestStatus.SENDED
          : user.friends.received.includes(friendId)
