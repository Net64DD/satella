...
import { createControllerPak, deleteControllerPak, downloadControllerPak, getControllerPaks, updateControllerPak, uploadControllerPak } from '../service/cpak.service';
...
export const createPak = async (req: Request, res: Response) => {
    ...
    try {
        return res.status(Responses.OK).json(
            await createControllerPak(session.ulid)
        );
    } catch (error) {
...
export const uploadPak = async (req: Request, res: Response) => {
    ...
    if (!pakId) {
        return res.status(Responses.BAD_REQUEST).json({ error: 'Invalid Controller Pak ID' });
    }
...
export const updatePak = async (req: Request, res: Response) => {
    ...
    if (!data.name && !data.access) {
        return res.status(Responses.BAD_REQUEST).json({ error: 'No data to update' });
    }
    try {
