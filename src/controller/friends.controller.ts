...
import { addFriend, getFriendsList, modifyFriendRequest, removeFriend, searchFriends } from '../service/friends.service';
...
export const addNewFriend = async (req: Request, res: Response) => {
    ...
    try {
        ...
        return res.status(Responses.OK).json({ message: 'Friend request sent successfully' });
    } catch (error) {
...
export const modifyUserRequest = async (req: Request, res: Response) => {
    ...
    if (!friendId || typeof accept !== 'boolean') {
        return res.status(400).json({ error: 'Friend ID and accept status are required' });
    }

    try {
        await modifyFriendRequest(session.ulid, friendId, accept);
        return res.status(Responses.OK).json({ message: 'Friend request modified successfully' });
    } catch (error) {
...
export const removeUserFriend = async (req: Request, res: Response) => {
    ...
    try {
        ...
        return res.status(Responses.OK).json({ message: 'Friend removed successfully' });
    } catch (error) {
